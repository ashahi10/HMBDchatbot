...
# --------------- LOGGING ----------------
logging.basicConfig(
    filename=f"description_migration_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
    filemode="w",
    level=logging.INFO,
...
def get_migration_stats(neo4j_connection: Neo4jConnection):
    ...
    WITH count(m) as total_with_desc
    OPTIONAL MATCH (m2:Metabolite)-[:HAS_DESCRIPTION]->(d:Description)
    WHERE m2.description IS NOT NULL
    RETURN total_with_desc,
           count(DISTINCT m2) as with_desc_node,
           (total_with_desc - count(DISTINCT m2)) as need_migration
    """
...
def cleanup_existing_issues(neo4j_connection: Neo4jConnection):
    ...
    # 1. Remove orphaned Description nodes
    orphan_query = """
    MATCH (d:Description)
    ...
    if orphaned > 0:
        print(f"   ✅ Removed {format_number(orphaned)} orphaned Description nodes")
    
    # 2. Fix duplicate relationships - keep only one per metabolite
    duplicate_query = """
    MATCH (m:Metabolite)-[r:HAS_DESCRIPTION]->(d:Description)
    WITH m, collect(r) as rels, collect(d) as descriptions
    WHERE size(rels) > 1
    FOREACH (i IN range(1, size(rels)-1) | 
        DELETE rels[i]
    )
    WITH m, descriptions
    FOREACH (i IN range(1, size(descriptions)-1) | 
    ...
    try:
        ...
        alt_query = """
        MATCH (m:Metabolite)
        WHERE size([(m)-[:HAS_DESCRIPTION]->() | 1]) > 1
        WITH m
        MATCH (m)-[r:HAS_DESCRIPTION]->(d:Description)
        WITH m, r, d
        ORDER BY d.descriptionId
        WITH m, collect(r)[1..] as extra_rels, collect(d)[1..] as extra_descs
        FOREACH (rel IN extra_rels | DELETE rel)
        FOREACH (desc IN extra_descs | DELETE desc)
        RETURN count(DISTINCT m) as cleaned
        """
        result = neo4j_connection.run_query(alt_query)
        cleaned = result[0]['cleaned'] if result else 0
        if cleaned > 0:
            print(f"   ✅ Fixed {format_number(cleaned)} duplicate relationships (alt method)")
    
    # 3. Fix mismatched descriptions
    mismatch_query = """
    MATCH (m:Metabolite)-[:HAS_DESCRIPTION]->(d:Description)
    WHERE m.description IS NOT NULL AND m.description <> d.text
    SET d.text = m.description,
        d.metabolite_id = m.accession,
        d.descriptionId = m.accession + '_desc',
        d.metabolite_element_id = elementId(m)
    RETURN count(*) as updated
    """
    result = neo4j_connection.run_query(mismatch_query)
    mismatched = result[0]['updated'] if result else 0
    if mismatched > 0:
        print(f"   ✅ Fixed {format_number(mismatched)} mismatched descriptions")
...
def ensure_constraints(neo4j_connection: Neo4jConnection):
    ...
    try:
        constraint_query = "CREATE CONSTRAINT IF NOT EXISTS FOR (d:Description) REQUIRE d.descriptionId IS UNIQUE"
        neo4j_connection.run_query(constraint_query)
        print("✅ Description constraint verified")
    except Exception as e:
        print(f"⚠️  Warning: Could not create Description constraint: {e}")
...
def process_migration_batch(neo4j_connection: Neo4jConnection, batch_size: int):
    ...
    LIMIT $batch_size
    WITH collect(m) as metabolites
    UNWIND metabolites as m
    CREATE (d:Description {
        descriptionId: m.accession + '_desc',
        text: m.description,
        metabolite_id: m.accession,
...
def process_individual_fallback(neo4j_connection: Neo4jConnection, batch_size: int):
    ...
    WHERE m.description IS NOT NULL 
    AND NOT EXISTS((m)-[:HAS_DESCRIPTION]->(:Description))
    RETURN m.accession as acc, m.description as desc
    ORDER BY m.accession
    LIMIT $batch_size
    """
    ...
    try:
        ...
        for record in records:
            ...
            try:
                result = neo4j_connection.run_query(individual_query, {"acc": acc, "desc": desc})
...
def migrate_descriptions(neo4j_connection: Neo4jConnection):
    """
    ...
    """
    print("\n🔍 Starting COMPREHENSIVE description migration...")
    logging.info("Starting comprehensive description migration")
    ...
    if total_to_migrate <= 0:
        print("✅ No metabolites need description migration")
        return
    
    print(f"📊 Migration Overview:")
    print(f"   • Total with descriptions: {format_number(stats['total_with_desc'])}")
    print(f"   • Already migrated: {format_number(stats['with_desc_node'])}")
